...
class Individual:
    def __init__(self, genes=None):
        ...
        if genes is None:
            ...
            gense1,gense2,gense3 = genes[:group],genes[group:2*group],genes[2*group:3*group]
    ...
    def evaluate_fitness(self):
        ...
        for i in range(group):
            w_idx = self.genes[i]
            n_idx = self.genes[i + group]
            g_idx = self.genes[i + 2*group]
            true_youxi = w_idx - n_idx - 2 * g_idx
...
class Ga:
    ...
    def cross(self):                           # 交叉
        ...
        for i in range(0, 99):
            ...
            index1 = random.randint(0, group-2)           # 左闭右闭
            index2 = random.randint(index1, group-1)
            index3 = random.randint(group, 2*group-2)  # 左闭右闭
            index4 = random.randint(index3, 2*group-1)
            index5 = random.randint(2*group, 3*group-2)  # 左闭右闭
            index6 = random.randint(index5, 3*group-1)
    ...
    def mutate(self, new_gen):        # 变异
        for individual in new_gen:
            if random.random() < self.pro_mutate:
                ...
                index1 = random.randint(0,group-2)
                index2 = random.randint(index1, group-1)
                genes_mutate1 = old_genes[index1:index2]    # 半开区间，左闭，右开
                genes_mutate1.reverse()

                index3 = random.randint(group, 2*group-2)
                index4 = random.randint(index3, 2*group-1)
                genes_mutate2 = old_genes[index3:index4]
                genes_mutate2.reverse()

                index5 = random.randint(2*group, 3*group-2)
                index6 = random.randint(index5, 3*group-1)
                genes_mutate3 = old_genes[index5:index6]
                genes_mutate3.reverse()

                individual.genes = old_genes[0:index1] + genes_mutate1 + old_genes[index2:group]+old_genes[group:index3] + genes_mutate2 + old_genes[index4:2*group]+old_genes[2*group:index5] + genes_mutate3 + old_genes[index6:]
        # 两代合并
...
print("GA"+str(len(w))+"规模的运行时间为"+str(round(runTime,2)))
...
for b in range(group):
    value1 = result[b]-result[b+group] - 2*result[b+2*group]
    if 10 < value1 < 40:
        T+=1
    value.append(value1)
...
for a in value:
    ...
    compare += abs(a-25)
print("平均游隙差值：",compare/group, "合套率：",T/group)
print("平均游隙差值(平方)：",compare_sqrt/group, "合套率：",T/group)
